...
class ActorNetwork(nn.Module):
    ...
    def forward(self, observation):
        ...
        # --------------
        # continues
        dist = torch.softmax(self.fc3(x),dim=1)
        dist = Categorical(dist)
        return dist
...
class PPO(object):
    ...
    def remember(self, state, action, reward,probs, vals, terminal):
        self.replay_buffer.store(obs=state, action=action, probs=probs, vals=vals, reward=reward, done=terminal)
